      ...
      <tr class="noborder"> 
        <td colspan="2" class="vatop"><div class="home-templates-board-layout style-<?php echo $output['web_array']['style_name'];?>">
            <div class="left">
              ...
              <?php if (is_array($output['code_recommends_list']['code_info']) && !empty($output['code_recommends_list']['code_info'])) { ?>
                <?php foreach ($output['code_recommends_list']['code_info'] as $key => $val) { ?>
                <dl recommends_id="<?php echo $key;?>">
                <dt>
                  <h4>热卖促销</h4>
                  <a href="JavaScript:show_recommends_dialog(<?php echo $key;?>);"><i class="icon-picture"></i><?php echo $lang['nc_edit'];?></a></dt>
                  
                  <?php if(!empty($val['goods_list']) && is_array($val['goods_list'])) { ?>
                    <dd class="act-hot-goods">
                    <ul>
                      <?php foreach($val['goods_list'] as $k => $v) { ?>
                      <li title="<?php echo $v['goods_name'];?>">
                          <a href="javascript:void(0);"><?php echo $v['goods_name'];?></a>
                       </li>
                      <?php } ?>
                       </ul>
                    </dd>
                ...
                <dd class="act-hot-goods">
                  	<ul>
                    	<li title="请添加数据"><a href="javascript:void(0);">请添加数据</a></li>
                        <li title="请添加数据"><a href="javascript:void(0);">请添加数据</a></li>
                    </ul>
                </dd>
                <?php } ?>
              ...
              <?php } ?>
                <?php } ?>
                <dd class="act-hot-goods" id="btn_adds_list">
                  	<ul>
                    	<a href="JavaScript:add_recommends();"><i class="icon-plus-sign-alt"></i> <?php echo '新增热卖促销模块';?></a>
                    </ul>
                </dd>
              
            </div>
            
            <div style=" float:right; width:720px; height:95px; display:block">
            <dl>
                <dt>
...
<!-- 热卖促销模块 -->
<div id="recommends_list_dialog" style="display:none;">
  <form id="recommends_list_form">
    <input type="hidden" name="web_id" value="<?php echo $output['code_recommends_list']['web_id'];?>">
    <input type="hidden" name="code_id" value="<?php echo $output['code_recommends_list']['code_id'];?>">
    <div id="recommends_input_list" style="display:none;"><!-- 推荐拖动排序 --></div>
    <?php if (is_array($output['code_recommends_list']['code_info']) && !empty($output['code_recommends_list']['code_info'])) { ?>
    <?php foreach ($output['code_recommends_list']['code_info'] as $key => $val) { ?>
            <dl select_recommends_id="<?php echo $key;?>">
              <dt>
                <h4 class="dialog-handle-title"><?php echo '热卖促销模块标题名称';?></h4>
                <div class="dialog-handle-box"><span class="left">
                  <input name="recommends_list[<?php echo $key;?>][recommends][name]" value="<?php echo $val['recommends']['name'];?>" type="text" class="w200">
                  </span><span class="right"><?php echo '修改该区域热卖促销商品模块选项卡名称，控制名称字符在4-8字左右，超出范围自动隐藏';?></span>
                  <div class="clear"></div>
                </div>
              </dt>
              <dd>
                <h4 class="dialog-handle-title"><?php echo $lang['web_config_recommend_goods'];?></h4>
                  <div class="s-tips"><i></i><?php echo '小提示：单击查询出的商品选中，双击已选择的可以删除，最多1个，保存后生效。';?></div>
                <ul class="dialog-goodslist-s1 goods-list">
                  <?php if(!empty($val['goods_list']) && is_array($val['goods_list'])) { ?>
                      <?php foreach($val['goods_list'] as $k => $v) { ?>
                      <li id="select_recommends_<?php echo $key;?>_goods_<?php echo $k;?>">
                        <div ondblclick="del_recommends_goods(<?php echo $v['goods_id'];?>);" class="goods-pic">
                        <span class="ac-ico" onclick="del_recommends_goods(<?php echo $v['goods_id'];?>);"></span> <span class="thumb size-72x72"><i></i><img select_goods_id="<?php echo $v['goods_id'];?>" title="<?php echo $v['goods_name'];?>" src="<?php echo strpos($v['goods_pic'],'http')===0 ? $v['goods_pic']:UPLOAD_SITE_URL."/".$v['goods_pic'];?>" onload="javascript:DrawImage(this,72,72);" /></span></div>
                        <div class="goods-name"><a href="<?php echo SHOP_SITE_URL."/index.php?act=goods&goods_id=".$v['goods_id'];?>" target="_blank"><?php echo $v['goods_name'];?></a></div>
                        <input name="recommends_list[<?php echo $key;?>][goods_list][<?php echo $v['goods_id'];?>][goods_id]" value="<?php echo $v['goods_id'];?>" type="hidden">
                        <input name="recommends_list[<?php echo $key;?>][goods_list][<?php echo $v['goods_id'];?>][market_price]" value="<?php echo $v['market_price'];?>" type="hidden">
                        <input name="recommends_list[<?php echo $key;?>][goods_list][<?php echo $v['goods_id'];?>][goods_name]" value="<?php echo $v['goods_name'];?>" type="hidden">
                        <input name="recommends_list[<?php echo $key;?>][goods_list][<?php echo $v['goods_id'];?>][goods_price]" value="<?php echo $v['goods_price'];?>" type="hidden">
                        <input name="recommends_list[<?php echo $key;?>][goods_list][<?php echo $v['goods_id'];?>][goods_pic]" value="<?php echo $v['goods_pic'];?>" type="hidden">
                      </li>
                      <?php } ?>
                      <?php } ?>
                </ul>
    ...
    <?php } ?>
    <?php } ?>
    <div id="add_recommends_list" style="display:none;"></div>
    <h4 class="dialog-handle-title"><?php echo $lang['web_config_recommend_add_goods'];?></h4>
    <div class="dialog-show-box">
    <table class="tb-type1 noborder search">
      <tbody>
        <tr>
          <th><label><?php echo $lang['web_config_recommend_gcategory'];?></label></th>
          <td class="dialog-select-bar" id="recommends_gcategory">
		        <input type="hidden" id="cate_id" name="cate_id" value="0" class="mls_id" />
		        <input type="hidden" id="cate_name" name="cate_name" value="" class="mls_names" />
		        <select>
		          <option value="0">-<?php echo $lang['nc_please_choose'];?>-</option>
        ...
        <tr>
          <th><label for="recommends_goods_name"><?php echo $lang['web_config_recommend_goods_name'];?></label></th>
          <td><input type="text" value="" name="recommends_goods_name" id="recommends_goods_name" class="txt">
		        <a href="JavaScript:void(0);" onclick="get_recommends_goods();" class="btn-search " title="<?php echo $lang['nc_query'];?>"></a>
          	</td>
        </tr>
      </tbody>
    </table>
      <div id="show_recommends_goods_list" class="show-recommends-goods-list"></div>
      <div class="clear"></div>
    </div>
    <div class="clear"></div>
    <a href="JavaScript:void(0);" onclick="update_recommends();" class="btn"><span><?php echo $lang['web_config_save'];?></span></a>
  </form>
</div>
