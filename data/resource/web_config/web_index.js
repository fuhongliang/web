  ...
  var recommends_max = 1;//热卖促销数
  var goods_max = 6;//商品数
  var goods_maxs = 2;//热卖促销数
  var brand_max = 8;//品牌限制
  var recommend_show = 2;//当前选择的商品推荐
  var recommends_show = 2;//当前选择的商品热卖促销
...
$(function(){
    ...
    //随Y轴滚动固定层定位
    $('.right').waypoint(function(event, direction) {
    	$(this).parents().toggleClass('sticky', direction === "down");
...
function update_dialog(id) {//初始化数据
	switch (id) {
		...
		case "recommend_list":
			...
			break;
		case "recommends_list":
			gcategoryInit("recommends_gcategory");
			$("#recommends_list_form dl dd ul").sortable({ items: 'li' });
			break;
		case "brand_list":
...
function del_recommend(id) {//删除商品推荐
	...
	if(confirm('您确定要删除吗?')) {
		$(".right dl[recommend_id='"+id+"']").remove();
...
function add_recommend() {//增加商品推荐
	for (var i = 1; i <= recommend_max; i++) {//防止数组下标重复
		if ($(".right dl[recommend_id='"+i+"']").size()==0) {//编号不存在时添加
			var add_html = '';
			var del_append = '';
...
function update_recommend() {//更新
	...
	if (get_text=='1') {
		...
		$(".right dl[recommend_id='"+id+"'] dt h4").html(recommend_name);
	  ...
	  $(".right dl[recommend_id='"+id+"'] dd").html('<ul class="goods-list">'+text_append+'</ul>');
...
function show_recommend_pic_dialog(id) {//弹出框
    ...
    if(obj.find("div[select_recommend_pic_id='"+id+"']").size()==0) {//不存在时添加
        var text_append = '<div select_recommend_pic_id="'+id+'" class="middle-banner">'+
							'<a href="javascript:void(0);" recommend_pic_id="11" class="left-a">140*140</a>'+
							'<a href="javascript:void(0);" recommend_pic_id="12" class="left-b">140*140</a>'+
							...
							'<a href="javascript:void(0);" recommend_pic_id="31" class="bottom-a">140*140</a>'+
							'<a href="javascript:void(0);" recommend_pic_id="32" class="bottom-b">140*140</a>'+
							'<a href="javascript:void(0);" recommend_pic_id="33" class="bottom-c">140*140</a>'+
							'<a href="javascript:void(0);" recommend_pic_id="34" class="bottom-d">140*140</a>'+
            	  '</div>';
...
function recommend_pic(pic_id,pic_img) {//更新图片
	if (pic_img!='') {
        ...
        $(".right dl[recommend_id='"+id+"'] dd").html('<div class="middle-banner">'+text_append+'</div>');//页面中的图片显示
        $(".right dl[recommend_id='"+id+"'] dd div.middle-banner a").css("opacity","");
	    $(".right dl[recommend_id='"+id+"'] dt h4").html(recommend_name);
	}
}
//切换图片上传
...
//热卖促销相关
function show_recommends_dialog(id) {//弹出框
	recommends_show = id;
	$("dl[select_recommends_id]").hide();
	$("dl[select_recommends_id='"+id+"']").show();
	show_dialog('recommends_list');
}
function get_recommends_goods() {//查询商品
	var gc_id = 0;
	$('#recommends_gcategory > select').each(function() {
		if ($(this).val()>0) gc_id = $(this).val();
	});
	var goods_name = $.trim($('#recommends_goods_name').val());
	if (gc_id>0 || goods_name!='') {
		$("#show_recommends_goods_list").load('index.php?act=web_api&op=recommends_list&'+$.param({'id':gc_id,'goods_name':goods_name }));
	}
}
function del_recommends(id) {//删除商品推荐
    if ($(".right dl").size()<1) {
         return;//保留一个
    }
	if(confirm('您确定要删除吗?')) {
		$(".right dl[recommends_id='"+id+"']").remove();
		$("input[name='recommends_list["+id+"]']").remove();
		$("dl[select_recommends_id='"+id+"']").remove();
		$("dl[select_recommends_pic_id='"+id+"']").remove();
		$("div[select_recommends_pic_id='"+id+"']").remove();
		update_data("recommends_list");//更新数据
	}
}
function add_recommends() {//增加商品推荐
	for (var i = 1; i <= recommends_max; i++) {//防止数组下标重复
		if ($(".left dl[recommends_id='"+i+"']").size()==0) {//编号不存在时添加
			var add_html = '';
			var del_append = '';
			del_append = '';//删除
			add_html = '<dl recommends_id="'+i+'"><dt><h4>热卖促销</h4>'+del_append+
    			'<a href="JavaScript:show_recommends_dialog('+i+');"><i class="icon-shopping-cart"></i>编辑</a></dt>'+
    			'<dd class="act-hot-goods"><ul><li><a href="javascript:void(0);">请添加数据</a></li><li><a href="javascript:void(0);">请添加数据</a></li></ul></dd></dl>';
			$("#btn_adds_list").before(add_html);
			$("#add_recommends_list").before('<dl select_recommends_id="'+i+'"><dt><h4 class="dialog-handle-title">热卖促销模块标题名称</h4>'+
    			'<div class="dialog-handle-box"><span class="left"><input name="recommends_list['+i+'][recommends][name]" value="热卖促销" type="text" class="w200"></span>'+
    			'<span class="right">修改该区域热卖促销商品模块选项卡名称，控制名称字符在4-8字左右，超出范围自动隐藏</span>'+
    			'<div class="clear"></div></div></dt><dd><h4 class="dialog-handle-title">推荐商品</h4><ul class="dialog-goodslist-s1 goods-list">'+
    			'<div class="s-tips"><i></i>小提示：单击查询出的商品选中，双击已选择的可以删除，最多1个，保存后生效。</div></ul></dd></dl>');
			$("#recommends_list_form dl dd ul").sortable({ items: 'li' });
			break;
		}
	}
}
function select_recommends_goods(goods_id) {//商品选择
	var id = recommends_show;
	var obj = $("dl[select_recommends_id='"+id+"']");
	if(obj.find("img[select_goods_id='"+goods_id+"']").size()>0) return;//避免重复
	if(obj.find("img[select_goods_id]").size()>=goods_maxs) return;
	var goods = $("#show_recommends_goods_list img[goods_id='"+goods_id+"']");
	var text_append = '';
	var goods_pic = goods.attr("src");
	var goods_name = goods.attr("title");
	var goods_price = goods.attr("goods_price");
	var market_price = goods.attr("market_price");
	text_append += '<div ondblclick="del_recommends_goods('+goods_id+');" class="goods-pic">';
	text_append += '<span class="ac-ico" onclick="del_recommends_goods('+goods_id+');"></span>';
	text_append += '<span class="thumb size-72x72">';
	text_append += '<i></i>';
  	text_append += '<img select_goods_id="'+goods_id+'" title="'+goods_name+'" src="'+goods_pic+'" onload="javascript:DrawImage(this,72,72);" />';
	text_append += '</span></div>';
	...
	text_append += '<a href="'+SHOP_SITE_URL+'/index.php?act=goods&goods_id='+goods_id+'" target="_blank">';
  	text_append += goods_name+'</a>';
	text_append += '</div>';
	text_append += '<input name="recommends_list['+id+'][goods_list]['+goods_id+'][goods_id]" value="'+goods_id+'" type="hidden">';
	text_append += '<input name="recommends_list['+id+'][goods_list]['+goods_id+'][market_price]" value="'+market_price+'" type="hidden">';
	text_append += '<input name="recommends_list['+id+'][goods_list]['+goods_id+'][goods_name]" value="'+goods_name+'" type="hidden">';
	text_append += '<input name="recommends_list['+id+'][goods_list]['+goods_id+'][goods_price]" value="'+goods_price+'" type="hidden">';
	text_append += '<input name="recommends_list['+id+'][goods_list]['+goods_id+'][goods_pic]" value="'+replace_url(goods_pic)+'" type="hidden">';
	obj.find("ul").append('<li id="select_recommends_'+id+'_goods_'+goods_id+'">'+text_append+'</li>');
}
function del_recommends_goods(goods_id) {//删除已选商品
	var id = recommends_show;
	var obj = $("dl[select_recommends_id='"+id+"']");
	obj.find("img[select_goods_id='"+goods_id+"']").parent().parent().parent().remove();
}
function update_recommends() {//更新
    var id = recommends_show;
    $("li[id^='select_recommends_"+id+"_pic_']").remove();
    $("div[select_recommends_pic_id='"+id+"']").remove();
	var get_text = update_data("recommends_list");
	if (get_text=='1') {
	    var obj = $("dl[select_recommends_id='"+id+"']");
		var text_append = '';
		var recommends_name = obj.find("dt input").val();
		$(".left dl[recommends_id='"+id+"'] dt h4").html(recommends_name);
		obj.find("img").each(function() {
			var goods = $(this);
			var goods_pic = goods.attr("src");
			var goods_name = goods.attr("title");
			text_append += '<li><a href="javascript:void(0);">'+goods_name+'</a></li>';
		});
	  $("dl[recommends_id='"+id+"'] dd ul").html('');
	  $(".left dl[recommends_id='"+id+"'] dd").html('<ul>'+text_append+'</ul>');
		DialogManager.close("recommends_list");
	}
}